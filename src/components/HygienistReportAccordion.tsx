...
interface HygienistReportAccordionProps {
  hygienistReport: HygienistReport;
  config: QueryConfig;
}

const HygienistReportAccordion = ({
  hygienistReport,
  config,
}: HygienistReportAccordionProps) => {
  ...
  useEffect(() => {
    if (isExpanded && !hygienistAppointments) {
      setLoading(true);
      getHygienistAppointments(hygienistReport, config);
    }
  ...
  return (
    <AccordionItem key={hygienistReport.ProvHyg}>
      {({ isExpanded }) => (
        <>
          <AccordionButton
          ...
          >
            <Grid templateColumns="1fr 1fr 1fr 1fr 20px" w="100%" px="10px">
              <Text
                textAlign="left"
                fontWeight={300}
              >{`${hygienistReport.FName} ${hygienistReport.LName}`}</Text>
              <Text textAlign="left" fontWeight={300}>
                {hygienistReport.CompletedAppts}
              </Text>
              <Text textAlign="left" fontWeight={300}>
                ${hygienistReport.Pay}
              </Text>
              <Text
                textAlign="left"
                fontWeight={300}
              >{`${hygienistReport.ReappointmentRate.toFixed(2)}%`}</Text>
...
export default HygienistReportAccordion;
